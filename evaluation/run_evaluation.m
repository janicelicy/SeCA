...
addpath('../XMLtools'); % path to LabelMe Toolbox
path = 'Images/';
seathru_set  = 'D3';
listing = dir(path);
...
[v, xml] = loadXML('Annotations/dkg_reference.xml');  
name = {v.annotation.object.name};
reference = zeros(18,3); 
I_ref = im2double(imread('Annotations/DKG_reference.png'));
[m,n,~] = size(I_ref); 
watermask = zeros(m,n);
...
for neutral_count =  7:24        
    for file_count = [48:numel(file_name_list)] %[start_point+1:46]%, 
        ...
        [v, xml] = loadXML(sprintf('Annotations/%s.xml', file_name));
